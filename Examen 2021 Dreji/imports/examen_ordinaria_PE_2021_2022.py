...
def simulate_continuous_time_Markov_Chain(
    ...
    # We can't compute the trayectories
    # in parallel because each trayectory will have a different
    # number of steps
    for _ in range(M):
        # Initialize each trayectory. 
        t = t0
        state = state_0
        new_times = [t0]
        ...
        while True:
            # Compute the time of the next jump
            t += np.random.exponential(scale=1.0/lambda_rates[state])
            
            # If we are out of time, end the simulation
            # of the trayectory
            if t > t1:
                break
            
            # Use the collapse function to compute which state
            # are we jumping to.
            collapsed_state = collapse(transition_matrix[state])
            state = np.argmax(collapsed_state == 1)

            # Save the actual time and trayectory state.
            new_times.append(t)
            new_trajectory.append(state)

        # Save the new trayectory
